...
在追蹤清單頁新增「跨裝置同步（選配）」設定：使用者輸入 email 後，系統自動建立雲端空間並產生同步金鑰（access token），追蹤清單即可在多裝置間自動同步；**未啟用同步的裝置行為與現況完全一致**（localStorage 為主、offline-first）。同步失敗時本地資料永不消失，另有匯出/匯入作為手動備援。
...
| 入口 | 位置 | 說明 |
|------|------|------|
| 🔄 同步設定區塊 | 追蹤清單頁（`/watchlist`） | 未啟用時顯示「跨裝置同步（選配）」說明＋email 輸入框 |
| 🔄 同步狀態列 | 追蹤清單頁頂部 | 已啟用時顯示狀態（同步中／已同步／失敗）＋上次同步時間 |
...
- ☐ 想要同步的每台裝置皆需輸入相同的 email（或貼上已產生的 token）
- ☐ 同步為選配——未啟用也能完整使用追蹤清單所有既有功能
...
| **操作前** | 使用者尚未啟用同步，追蹤清單儲存於本機瀏覽器 |
| **系統回應** | 顯示追蹤清單與「🔄 跨裝置同步（選配）」區塊，含說明文字與 email 輸入框 |
| **操作後** | 追蹤清單所有既有操作（搜尋、加入、移除、切換模式）行為與先前完全一致 |
| **下一步** | 步驟 2：輸入 email 啟用；或忽略此區塊繼續正常使用 |
...
### 步驟 6：已啟用裝置日常增刪追蹤（自動同步）

| | 描述 |
|---|------|
| **觸發** | 使用者在任一已啟用裝置上點擊 ❤️ 加入追蹤，或移除追蹤 |
| **操作前** | 裝置已啟用同步，狀態為「已同步」 |
| **系統回應** | 本機清單立即更新（❤️ 變色、徽章增減）；約 1.5 秒後自動寫回雲端，狀態短暫顯示「同步中…」後回到「已同步」 |
| **操作後** | 變更已寫入雲端；本機行為與未啟用時完全相同（無感） |
| **下一步** | 步驟 7：在另一台裝置確認變更 |
...
| | 描述 |
|---|------|
| **觸發** | 使用者把另一台已啟用裝置切回追蹤清單頁（或該裝置前台每 60 秒自動檢查） |
...
| **觸發** | 使用者點擊狀態列的「停用」按鈕 |
| **操作前** | 已啟用同步，處於任一狀態 |
| **系統回應** | 停用確認後（依實作可能直接執行或需確認），狀態列變為「未啟用同步」，停止自動同步；localStorage 中的 token 保留（可重新啟用） |
| **操作後** | 本地追蹤清單完整保留；裝置回到與未啟用時完全相同的行為 |
| **下一步** | 步驟 12：日後想再同步可重新輸入 email 或使用已有 token |
...
| **操作後** | 本地追蹤清單包含匯入的股票；如該裝置已啟用同步，合併結果亦會自動同步上雲端 |
| **下一步** | 回到其他追蹤操作正常使用 |
...
| Token 外流 | 他人可能讀寫你的雲端清單 | 重新輸入 email 產生新 token（新 token 自動覆蓋舊 token） |
...
| 項目 | 說明 |
|------|------|
| 同步為選配 | 未啟用同步的裝置不啟動任何同步，行為與現況 100% 一致 |
| 免登入 | 輸入 email 即可使用；每個 email 對應一份雲端清單，他人無法讀到你的清單 |
...
| Token 時效 | TTL 90 天，過期需重新輸入 email 產生新 token |
| 匯出/匯入為手動備援 | 不走自動同步；僅搬移清單內容，不涉及同步狀態 |
...
- [ ] 重新整理頁面後仍維持已啟用狀態（token 被記住）

### 同步狀態顯示
- [ ] 已啟用時狀態列顯示：同步中…／已同步／同步失敗 其中一種
- [ ] 已同步時顯示上次同步時間
- [ ] 同步失敗時顯示錯誤訊息
- [ ] 429 時顯示「速率限制（429），N 秒後重試」及退避秒數

### 跨裝置同步
- [ ] 兩台裝置各自輸入相同 email 後，裝置 A 新增股票，裝置 B 切回頁面即可看到
