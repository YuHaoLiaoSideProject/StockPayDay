...
class TWSEStockCrawler:
    ...
    def fetch_dividend_list(self, year: int, quarter: int) -> List[Dict]:
        ...
        logger.info(
            "fetch_dividend_list(%d, Q%d) — 解析到 %d 筆資料",
    ...
    def _parse_html_table(self, html: str) -> List[Dict]:
        ...
        for row in rows[1:]:  # 跳過標題列
            ...
            # 跳過合計列或空列
            if not code or code == "合計" or not code.isdigit():
                continue
    ...
    def fetch_stock_dividends(
        ...
        Returns:
            ...
            - processed_stocks: 以 code 為 key 的字典列表，符合 data/stocks/{code}.json 格式
        """
        ...
        for stock in grouped.values():
            stock["dividend_history"].sort(key=lambda x: (x["year"], x["quarter"]), reverse=True)
