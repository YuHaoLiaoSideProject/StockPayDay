...
class TWSEETFCrawler:
    ...
    def fetch_etf_dividends_for_quarter(self, year: int, quarter: int) -> List[Dict]:
        ...
        # 5. 解析 HTML 表格，篩選 ETF
        records = self._parse_html_table_for_etf(resp.text)
        logger.info(
            "fetch_etf_dividends_for_quarter(%d, Q%d) — 解析到 %d 筆 ETF",
    ...
    def _parse_html_table_for_etf(self, html: str) -> List[Dict]:
        """
        解析 MOPS 回傳 HTML 中的配息資料表格，篩選 ETF。
    ...
    def fetch_etf_dividends(
        ...
        Returns:
            ...
            - processed_etfs: 以 code 為 key 的字典列表，符合 data/etfs/{code}.json 格式
        """
        ...
        for etf in grouped.values():
            etf["dividend_history"].sort(
